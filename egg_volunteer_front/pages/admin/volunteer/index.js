...
Page({
  ...
  onViewEnrollment(e) {
    wx.navigateTo({
      url: './signlist?id=' + e.currentTarget.dataset.id,
    })
  },
  ...
  onDeleteActivity(e) {
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          wx.request({
            url: config.host + '/api/admin/activity/delete?id=' + e.currentTarget.dataset.id,
            method: 'DELETE',
            header: {
              'Authorization': wx.getStorageSync('token')
            },
