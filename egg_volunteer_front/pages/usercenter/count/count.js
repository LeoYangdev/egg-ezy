...
Page({
  ...
  pageLoad() {
    ...
    if (this.data.currenTab == 0) {
      // 获取志愿数据
      wx.request({
        url: host+'/api/total/activity/index',
        method: 'get',
        header: {
        ...
        success: res=>{
          ...
          this.setData({
            ...
            total_vla_duration: res.data.data.total_duration,
            total_vla_num: res.data.data.total_num
          });
        },
        fail: res=>{
          Toast({
      ...
      wx.request({
        ...
        success: res=>{
          ...
          this.setData({
            ...
            currenTotal: res.data.data.total,
            works: processData,
            total_work_duration: res.data.data.total_duration,
            total_work_num: res.data.data.total_num
          });
        },
        fail: res=>{
          Toast({
  ...
  search: _.debounce(function(e) {
    ...
    if (this.data.currenTab == 0) {
      // 获取志愿数据
      wx.request({
        url: host+'/api/total/activity/search?key=' + key,
        method: 'get',
        header: {
          'Authorization': wx.getStorageSync('token')
        },
      ...
      // 获取work数据
      wx.request({
        url: host+'/api/total/work/search?key=' + key,
        method: 'get',
        header: {
          'Authorization': wx.getStorageSync('token')
        },
  ...
  onReachBottom(){
    ...
    if (this.data.currenTab == 0) {
      // 获取志愿数据
      wx.request({
        url: host+'/api/total/activity/index?page=' + page,
        method: 'get',
        header: {
          'Authorization': wx.getStorageSync('token')
        },
      ...
      wx.request({
        url: host+'/api/total/work/index?page=' + page,
        method: 'get',
        header: {
          'Authorization': wx.getStorageSync('token')
        },
